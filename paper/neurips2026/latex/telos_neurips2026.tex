...
\paragraph{Motivation for whitening.}
Na\"ive mean-difference extraction (standard CAA) yields ${\sim}99.2\%$ cosine
similarity between harmful and harmless activation clusters at mid-layers in
Llama-3.1-8B---the ethical signal is effectively invisible in the raw feature
space.
Inspection reveals that a small number of syntactic-role dimensions reach
activation magnitudes of ${\sim}326$ (versus ${\sim}1$--$3$ for semantic
features), dominating the covariance and masking the refusal direction.
Whitening suppresses these high-variance outlier dimensions, revealing a
real separation gap of $0.28$--$0.51$ in cosine distance---the gap that drives
all detection and steering downstream.
Without this step, the extracted direction is an artifact of syntactic
co-variation rather than a representation of refusal intent.
...
\textbf{Pre-v2.1 --- Layer timing and component failures (XP-06/07/08).}
Early iterations revealed two structural limits.
First, \textit{semantic crystallization}: intervening at Layer~19 (the final
attention layers of Llama-3.1-8B) produced no measurable effect---by that depth,
next-token probability is already committed and the output distribution is
effectively frozen.
This established mid-layer interception ($L_{12}$--$L_{22}$) as the necessary
intervention window.
Second, \textit{context amnesia}: a KV-Cache decay component (decay $= 0.30$)
intended to neutralize adversarial prefixes erased the model's reasoning context
entirely, yielding 100\% incoherent outputs---removing the adversarial prefix
simultaneously removed the capacity to respond coherently.
Both failures converged on a single design principle: \textit{eliminate components
that trade reasoning capacity for safety gains}.
...
% -----------------------------------------------------------------------
\subsection{Governance Effectiveness as a Function of Representational Capacity}
\label{subsec:representational_capacity}

A consistent pattern across the cross-model evaluation reveals that the
effectiveness of latent governance is bounded by the \textit{representational
capacity} of the target model---specifically, by whether the model's latent
geometry separates malicious from benign technical concepts into distinct
subspaces.

\paragraph{The latent entanglement hypothesis.}
In models with low hidden dimension ($d = 2048$, Qwen3-4B), the residual stream
must encode a wide range of concepts in a compressed space. As a consequence,
semantically adjacent domains---such as \textit{cybercrime exploitation} and
\textit{legitimate code generation}---share overlapping directions in the
refusal subspace. We term this \textit{latent entanglement}. When TEL-OS steers
toward the refusal direction at a given layer, it simultaneously suppresses
signal from both the malicious and the benign concept, placing the model in a
semantic limbo: it detects the threat (100\% vector triggered) but cannot
resolve the distinction between a harmful exploit request and a benign
programming task. The result is a high partial-response rate (38.25\%) and a
residual ASR floor (3.75\%) that cannot be reduced by increasing $\alpha$
without destroying benign utility.

\paragraph{Disentanglement as an emergent property of scale.}
As model scale increases, the hidden dimension expands ($d = 4096$ for
Llama-3.1-8B; $d = 5120$ for Qwen3-32B), providing additional degrees of
freedom for the model to encode domain-specific concepts in orthogonal
directions. The empirical consequence is a \textit{binary wall}: harmful
queries immediately saturate urgency at the maximum (urgency $= 3.0$), while
benign queries fall below the detection threshold, with negligible intermediate
mass. TEL-OS can then apply full steering strength without interfering with
benign representations.

Table~\ref{tab:rep_capacity} summarizes the three-point comparison from
real experimental data (XP-25c, XP-26, XP-27c).

\begin{table}[ht]
\centering
\caption{Governance effectiveness vs.\ representational capacity.
  All HarmBench Standard (400 behaviors), StrongREJECT (GPT-4o).
  \textit{Urgency mid} = fraction of behaviors with intermediate urgency
  $\in (1.0, 3.0)$ (entanglement indicator).
  Norm Drift $\approx 1.0000$ across all models by SLERP construction.}
\label{tab:rep_capacity}
\small
\begin{tabular}{lcccccc}
\toprule
\textbf{Model} & \textbf{Params} & $d$ & $\alpha_{\max}$ &
  \textbf{HarmBench ASR} & \textbf{Partial} & \textbf{Urgency mid} \\
\midrule
Qwen3-4B           & 4B  & 2048 & 0.35 & 3.75\%          & 38.25\% & ${\sim}10\%$ \\
Llama-3.1-8B       & 8B  & 4096 & 0.20 & \textbf{0.00\%} &  8.75\% & ${\sim}0\%$  \\
Qwen3-32B          & 32B & 5120 & 0.50 & \textbf{0.75\%} &  2.25\% & $0.0\%$      \\
\bottomrule
\end{tabular}
\end{table}

The partial-response rate is the most diagnostic signal: 38.25\% for Qwen3-4B
versus 8.75\% and 2.25\% for the larger models. This quantifies the steering
resolution available to the governance layer---a direct proxy for subspace
separation in the residual stream.

\paragraph{Implications.}
This finding reframes the Qwen3-4B 3.75\% result from a failure of TEL-OS to
a \textit{latent resolution limit}: the governance framework is operating at
the boundary of what the model's geometry permits. The same OBLITERATUS
extraction and SLERP steering that achieves 0.00\% on 8B+ models cannot exceed
the representational resolution of a 4B model. A corollary prediction follows:
as model scale continues to grow, latent entanglement between adjacent domains
will decrease, and TEL-OS governance effectiveness will monotonically improve
without any changes to the framework itself. The 4B--8B--32B trajectory in
Table~\ref{tab:rep_capacity} is consistent with this prediction.
...
A final distinction from prior defenses: TEL-OS does not \textit{block} harmful
outputs---it eliminates the model's geometric capacity to produce them.
When SLERP rotates a harmful activation toward the refusal manifold, the latent
state that would produce a harmful continuation no longer exists in the
representation space; the model is not ``choosing'' to refuse.
This \textit{representation disruption} is structurally distinct from
output filtering, detection-and-refusal, and weight-level alignment: the
intervention operates before the generation decision is formed, at the level
of the representation itself.
The flat AutoDAN loss landscape and the 100\% vector-trigger rate in adversarial
experiments are direct empirical signatures of this mechanism.
